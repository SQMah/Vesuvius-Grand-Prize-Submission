...
import gc
from training_unetr_pp import RegressionPLModel
...
class InferenceArgumentParser(Tap):
    ...
    size: int = 256
    stride: int = 32
    model_path: str = './models/model.ckpt'
        ...
        if accelerator.is_main_process:
            ...
            for i, (x1, y1, x2, y2) in enumerate(xys):
                mask_pred[y1:y2, x1:x2] += np.multiply(
                    F.interpolate(y_preds[i].unsqueeze(0).float(), scale_factor=1, mode='bilinear').squeeze(0).squeeze(
                        0).numpy(), kernel)
...
if __name__ == "__main__":
    ...
    model = RegressionPLModel.load_from_checkpoint(args.model_path, strict=False, train_segment_ids=[], val_segment_ids=[], stride=CFG.stride, size=CFG.size, depth=CFG.in_chans)
    ...
    for fragment in cfg.CFG.test_segment_ids:
        ...
        test_loader, test_xyxz, test_shape, fragment_mask = get_img_splits(fragment_id, args.start_idx, args.start_idx + 16,
                                                                           0)
        test_loader = accelerator.prepare_data_loader(test_loader)
        accelerator.print("Done getting splits!")
        accelerator.print(f"Running predict function for {fragment}")
        with autocast(True):
