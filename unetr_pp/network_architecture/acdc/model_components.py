...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=None ,dims=[32, 64, 128, 256],
                 proj_size =[64,64,64,32], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=3, in_channels=1,
                 dropout=0.0, transformer_dropout_rate=0.1, window_size=256, depth=16, **kwargs):
        super().__init__()
        if input_size is None:
            input_size = [depth * (window_size//4) * (window_size//4), depth//2 * (window_size//8) * (window_size//8), depth//4 * (window_size//16) * (window_size//16), depth//8 * (window_size//32) * (window_size//32)]
